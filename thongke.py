            ...
            if not selected_years:
                ...
                if total_rec_df.empty:
                    ...
                    if is_only_gd:
                        ...
                        # 🌟 ĐIỀU CHỈNH QUAN TRỌNG: Chuẩn hóa tầng cơ sở (df_clean_unified) 
                        # Chỉ gom theo Năm học và Sản phẩm chuẩn hóa để triệt tiêu việc khác vai trò/thành viên mà bị tách dòng
                        ...
                        # ==========================================
                        # 🧹 2.1 BẢNG THỐNG KÊ SAU KHI TRỪ TRÙNG LẶP (ĐÃ KHỚP TUYỆT ĐỐI)
                        # ==========================================
                        st.markdown("##### 🧹 2.1 Bảng thống kê SAU KHI trừ trùng lặp")
                        df_after = df_clean_unified.groupby("Năm học hiển thị").agg(**{
                            "Số lượng sản phẩm độc lập": ("Sản phẩm chuẩn hóa", "count"),
                        ...
                        # ==========================================
                        # 🔍 2.2 BẢNG CHI TIẾT TÙY CHỈNH TIÊU CHÍ 
                        # ==========================================
                        ...
                        middle_cols = [c for c in ["Số lượng", "Số lượng sau gom nhóm", "Tổng số tiết"] if c in df_nckh_detail.columns]
                        ...
                        if not df_nckh_detail.empty:
                            ...
                            for col_name in df_nckh_detail.columns:
                                ...
                                elif col_name == "Số lượng sau gom nhóm":
                                    total_row_nckh[col_name] = tot_sl_sau_gom
                                elif col_name == "Tổng số tiết":
                        ...
                        if not df_plot_nckh.empty:
                            ...
                            for col_name, display_name in allowed_mapping:
                                ...
                                if col_name != "Năm học hiển thị" and has_year_nckh:
                                    ...
                                    df_pivot_qty = df_nckh_filtered.groupby(col_name)[["Số lượng sau gom nhóm"]].sum()
                                ...
                                # 1. Biểu đồ Số lượng sau gom nhóm
                                with col_chart1:
