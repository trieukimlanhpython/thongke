            ...
            if not selected_years:
                ...
                if total_rec_df.empty:
                    ...
                    if is_only_gd:
                        ...
                        # 🌟 Nhận diện và xử lý tách thông tin "đợt" từ cột "code" (Ví dụ: 2526.D1 -> Đợt 1)
                        code_col_actual = next((c for c in df_clean.columns if "code" in c), None)
                        if code_col_actual:
                            # Tách lấy chữ cái/số sau dấu chấm hoặc chữ D (VD: .d1 -> Đợt 1)
                        ...
                        if opt_dot:
                            group_detail_keys.append(c_dot)  # 🌟 Thêm key Đợt học vào nhóm
                        ...
                        if c_dot:
                            rename_detail_dict[c_dot] = "Đợt học"  # 🌟 Đổi tên hiển thị Đợt học
                        if c_faculty:
                        ...
                        # ==========================================
                        # 📊 3. BIỂU ĐỒ TRỰC QUAN ĐỘNG (CỘT NHÓM THEO THỜI GIAN / NĂM HỌC, NÉT CAO, CHIA 2 PHẦN)
                        # ==========================================
                        ...
                        if not df_plot_data.empty:
                            ...
                            # Loại bỏ "Năm học" ra khỏi danh sách tiêu chí đơn lẻ bên dưới vì ta đã gộp chung vào biểu đồ cột nhóm so sánh theo thời gian
                            other_criteria_cols = [c for c in active_criteria_cols if c != "Năm học"]
    
                            # ==========================================
                            # 🌟 TRƯỜNG HỢP 1: CHỈ CHỌN MỘT TIÊU CHÍ (KHÔNG CÓ NĂM HỌC)
                            # ==========================================
                            for crit_col in other_criteria_cols:
                                ...
                                for idx, chunk_df in enumerate(chunks):
                                    ...
                                    dynamic_width = max(8.0, num_bars * 0.45)
                                    ...
                                    with col_c1:
                                        fig1, ax1 = plt.subplots(figsize=(dynamic_width, 4.2), dpi=300)
                                    ...
                                    with col_c2:
                                        fig2, ax2 = plt.subplots(figsize=(dynamic_width, 4.2), dpi=300)
                            ...
                            # ==========================================
                            # 🌟 TRƯỜNG HỢP 2: KHI CHỌN ĐỒNG THỜI NĂM HỌC VÀ CÁC TIÊU CHÍ KHÁC -> VẼ BIỂU ĐỒ CỘT NHÓM SO SÁNH QUA THỜI GIAN (NĂM HỌC)
                            # ==========================================
                            if has_year_selected and other_criteria_cols:
                                st.markdown("---")
                                st.markdown("#### 🌟 3.1 Biểu đồ so sánh chi tiết các tiêu chí theo thời gian (Năm học)")
                                ...
                                for other_col in other_criteria_cols:
                                    ...
                                    # Pivot dữ liệu với trục hoành là tiêu chí, các cột là Năm học (thời gian)
                                    ...
                                    pivot_tiet = pivot_tiet[(pivot_tiet != 0).any(axis=1)]
                                    pivot_lop = pivot_lop[(pivot_lop != 0).any(axis=1)]
                                    ...
                                    # Tự động chia đôi thành 2 phần liền mạch nếu quá dài
                                    ...
                                    for c_idx, (sub_tiet, sub_lop) in enumerate(zip(chunks_tiet, chunks_lop)):
                                        ...
                                        with col_g1:
                                            fig_g1, ax_g1 = plt.subplots(figsize=(dyn_w_grp, 4.2), dpi=300)
                                            ...
                                            ax_g1.set_title(f"Tổng số tiết - {other_col} (Phần {c_idx+1})", fontsize=10, fontweight="bold")
                                        ...
                                        with col_g2:
                                            fig_g2, ax_g2 = plt.subplots(figsize=(dyn_w_grp, 4.2), dpi=300)
                                            ...
                                            ax_g2.set_title(f"Số lượng lớp - {other_col} (Phần {c_idx+1})", fontsize=10, fontweight="bold")
